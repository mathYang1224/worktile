...
<template>
	<div>
		<div class="canlendar">
			...
			<div class="bg_box">
				<table>
					<thead>
						<tr>
							<th v-for="(item,index) in dayofweek">{{item}}</th>
						</tr>
					</thead>
					<tbody>
		...
		<!-- 每日信息框 -->
		<div id="dailyInfoBox" style="display:none;width:180px;height:424px;">
			<daily-info-box v-bind:dailyinfo="dailyinfo"></daily-info-box>
		</div>
	</div>
</template>
...
<script>
	...
	import calcDailySchedule from "./calcDailySchedule.js";
	import dailyInfoBox from "./dailyInfoBox.vue";
					...
					$(".bg_box table tbody tr td").each(function(){
							...
							if(left > L && left < R && top > T && top < B){
								...
								if(data.day != self.date){
									...
									self.date = data.day;
									let date = data.day;
								 	let dailyschedule = calcDailySchedule(year,month,date,self.themonth,self.prevmonth,self.nextmonth,self.$store.state.nowshow.year,self.$store.state.nowshow.month);
